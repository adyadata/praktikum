...
$master_waktu_delete = NULL; // Initialize page object first

class cmaster_waktu_delete extends cmaster_waktu {
	...
	// Page object name
	var $PageObjName = 'master_waktu_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("master_waktulist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in master_waktu class, master_waktuinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("master_waktulist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($master_waktu_delete)) $master_waktu_delete = new cmaster_waktu_delete();

// Page init
$master_waktu_delete->Page_Init();

// Page main
$master_waktu_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$master_waktu_delete->Page_Render();
?>
...
var CurrentForm = fmaster_waktudelete = new ew_Form("fmaster_waktudelete", "delete");

// Form_CustomValidate event
fmaster_waktudelete.Form_CustomValidate = 
...
fmaster_waktudelete.ValidateRequired = true;
<?php } else { ?>
fmaster_waktudelete.ValidateRequired = false; 
<?php } ?>
...
<?php $master_waktu_delete->ShowPageHeader(); ?>
<?php
$master_waktu_delete->ShowMessage();
?>
<form name="fmaster_waktudelete" id="fmaster_waktudelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($master_waktu_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $master_waktu_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="master_waktu">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($master_waktu_delete->RecKeys as $key) { ?>
...
<?php if ($master_waktu->id_waktu->Visible) { // id_waktu ?>
		<th><span id="elh_master_waktu_id_waktu" class="master_waktu_id_waktu"><?php echo $master_waktu->id_waktu->FldCaption() ?></span></th>
<?php } ?>
<?php if ($master_waktu->id_kelompok->Visible) { // id_kelompok ?>
		<th><span id="elh_master_waktu_id_kelompok" class="master_waktu_id_kelompok"><?php echo $master_waktu->id_kelompok->FldCaption() ?></span></th>
<?php } ?>
<?php if ($master_waktu->id_tanggal->Visible) { // id_tanggal ?>
		<th><span id="elh_master_waktu_id_tanggal" class="master_waktu_id_tanggal"><?php echo $master_waktu->id_tanggal->FldCaption() ?></span></th>
<?php } ?>
<?php if ($master_waktu->jam_mulai->Visible) { // jam_mulai ?>
		<th><span id="elh_master_waktu_jam_mulai" class="master_waktu_jam_mulai"><?php echo $master_waktu->jam_mulai->FldCaption() ?></span></th>
<?php } ?>
<?php if ($master_waktu->jam_selesai->Visible) { // jam_selesai ?>
		<th><span id="elh_master_waktu_jam_selesai" class="master_waktu_jam_selesai"><?php echo $master_waktu->jam_selesai->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$master_waktu_delete->RecCnt = 0;
$i = 0;
while (!$master_waktu_delete->Recordset->EOF) {
	$master_waktu_delete->RecCnt++;
	$master_waktu_delete->RowCnt++;

	// Set row properties
	$master_waktu->ResetAttrs();
	$master_waktu->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$master_waktu_delete->LoadRowValues($master_waktu_delete->Recordset);

	// Render row
	$master_waktu_delete->RenderRow();
?>
	<tr<?php echo $master_waktu->RowAttributes() ?>>
<?php if ($master_waktu->id_waktu->Visible) { // id_waktu ?>
		<td<?php echo $master_waktu->id_waktu->CellAttributes() ?>>
<span id="el<?php echo $master_waktu_delete->RowCnt ?>_master_waktu_id_waktu" class="master_waktu_id_waktu">
...
<?php } ?>
<?php if ($master_waktu->id_kelompok->Visible) { // id_kelompok ?>
		<td<?php echo $master_waktu->id_kelompok->CellAttributes() ?>>
<span id="el<?php echo $master_waktu_delete->RowCnt ?>_master_waktu_id_kelompok" class="master_waktu_id_kelompok">
...
<?php } ?>
<?php if ($master_waktu->id_tanggal->Visible) { // id_tanggal ?>
		<td<?php echo $master_waktu->id_tanggal->CellAttributes() ?>>
<span id="el<?php echo $master_waktu_delete->RowCnt ?>_master_waktu_id_tanggal" class="master_waktu_id_tanggal">
...
<?php } ?>
<?php if ($master_waktu->jam_mulai->Visible) { // jam_mulai ?>
		<td<?php echo $master_waktu->jam_mulai->CellAttributes() ?>>
<span id="el<?php echo $master_waktu_delete->RowCnt ?>_master_waktu_jam_mulai" class="master_waktu_jam_mulai">
...
<?php } ?>
<?php if ($master_waktu->jam_selesai->Visible) { // jam_selesai ?>
		<td<?php echo $master_waktu->jam_selesai->CellAttributes() ?>>
<span id="el<?php echo $master_waktu_delete->RowCnt ?>_master_waktu_jam_selesai" class="master_waktu_jam_selesai">
...
<?php
	$master_waktu_delete->Recordset->MoveNext();
}
$master_waktu_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $master_waktu_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fmaster_waktudelete.Init();
</script>
<?php
$master_waktu_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$master_waktu_delete->Page_Terminate();
?>
