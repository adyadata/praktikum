...
function CurrentProjectID() {
	...
	return "{47E9807F-0BA5-4478-84CF-DB02752CE563}";
}
...
class cAdvancedSecurity {
	...
	function SetUpUserLevelEx() {
		...
		if (count($this->UserLevelPriv) == 0 && $this->IsAdmin() && $Page != NULL && @$_SESSION[EW_SESSION_USER_LEVEL_MSG] == "") {
			...
			$Page->Page_Terminate("t_03_userlevelslist.php");
		}
		return TRUE;
	}
...
function Database_Connecting(&$info) {
	...
	if (ew_CurrentUserIP () == "127.0.0.1"  || ew_CurrentUserIP () == ":: 1"  || ew_CurrentHost () == "localhost" ) { // testing on local PC
		...
		$info["db"] = "sim_praktikum"; // sesuaikan dengan nama database di komputer localhost
	} elseif (ew_CurrentHost () == "praktikum.nma-indonesia.com") { // setting koneksi database untuk komputer server
		$info["host"] = "mysql.idhostinger.com";  // sesuaikan dengan ip address atau hostname komputer server
		$info["user"] = "u945388674_prakt"; // sesuaikan dengan username database di komputer server
		$info["pass"] = "M457r1P 81"; // sesuaikan deengan password database di komputer server
		$info["db"] = "u945388674_prakt"; // sesuaikan dengan nama database di komputer server
	}
}
